...
(function ($) {
    WDK.page('product/edit', "商品编辑", false, function (root) {

        // menu.html(root.find('#menu').text()).cls('right-menu-item', 1).find('button')
        //     .click(function () {
        //         var pro = $('form', root).val();
        //         if (!pro) {
        //             return false;

        //         }
        //         WDK.UI.Command("Settings", 'Product', pro);
        //     })

        root.on('event', function (e, k) {
            switch (k) {
                case 'save':
                    var pro = $('form', root).val();
                    if (!pro) {
        ...
        root.on("hash", function (e, v) {
            WDK.UI.Command("Data", 'pro', v.Id || 'News', function (pro) {
                proId = pro.Id;
