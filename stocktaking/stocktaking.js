(function ($) {
    ...
    WDK.tpl('stocktaking/product', "stocktaking/porder", function (root, menu) {
        ...
        var table = $('table', root).thead().on('change', 'input', function (e) {
            ...
            if (isNaN(p) == false) {
                WDK.UI.Command('Stocktaking', 'Product', { Id: me.attr('stocktaking_id'), product_id: me.attr('product_id'), sku_id: me.attr('sku_id'), Quantity: this.value });
        ...
        var paging = $('.pagination-container', root).paging("Stocktaking", "ProductSKU", $('table tbody', root), htmldata);


        root.on('hash', function (e, v) {
            WDK.UI.Command('Stocktaking', "Search", { Type: 'Product', Id: v.key }, function (xhr) {
            ...
            if (isEditer) {
                if (StocktakingId && v) {
                    WDK.UI.Command('Stocktaking', "ProductDetail", { product_id: v, Id: StocktakingId });
                    return false;
                }
            };
    ...
    }, '商品盘点单明细').tpl('stocktaking/store', 'stocktaking/sorder', function (root, title, menu) {
        menu.html(root.find('#menu').text()).cls('right-menu-item', 1);
        var isEditer = false;
        var StocktakingId;
        var inputHtml = $('#editer', root).text();
        ...
        root.on('hash', function (e, v) {
            WDK.UI.Command('Stocktaking', "Search", { Type: 'Store', Id: v.key }, function (xhr) {
                isEditer = xhr.IsEditer;
                StocktakingId = xhr.Id;
                if (xhr.ButtonText) {
                    menu.find('a').attr('send', StocktakingId).text(xhr.ButtonText).cls('hide', false);
