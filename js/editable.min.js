    ...
    function editer(dom) {
        ...
        tool.find('.el-editer-html-view').click(function () {
            ...
            if (pdom.is('.markdown')) {
                ...
                var m = $(this);
                m.addClass('loading')
                $.uploader(blob, function (xhr) {
                    $.UI.API('Subject', 'View', { Url: xhr.src, start: 0 }, d => {
                ...
                $.UI.Confirm('提示', '图文转化Markdown会丢失一此格式，确认转换吗', function (v) {
                    textarea.val($.UI.parseCMS(descDom[0], true));
        ...
        descDom.on('click', function (e) {
        ...
        }).on('WebResource', function (e, t) {
            tool.find('.cssUploader').removeClass('loading');
            ...
            tool.find('.cssUploader').addClass('loading');
            $.uploader(file, function (xhr) {
                $.UI.API('System', 'Resource', { media_id: xhr.src, root: descDom.attr('res-id') || '' })
            });
            ...
            switch (e.keyCode) {
                case 13:
                    var isb = true;
                    Check(function (x) {
                        var insertText = e.ctrlKey || e.metaKey;
                        if (!insertText && x.is('.wdk-cms-grid')) {
                            isb = false;
                            var node = window.getSelection().anchorNode;
                            var active = $(node.nodeType == 3 ? node.parentNode : node);
                            if (!active.is('td')) {
                                active = active.parent('td');
                            }
                            insertText = x.find('td').last()[0] == active[0];

                        }
                        if (insertText) {
                            isb = false;
                            insertText = tr
                            var text = $({ tag: 'div', cls: 'wdk-cms-text' })
                            x.after(text);
                    ...
                    Check(function (x) {
                        if (x.is('.wdk-cms-grid')) {
                            if (!x.text().replace(/\s+/g, '')) {
                                var em = x[0].nextElementSibling || x[0].previousElementSibling;
                                if (!em) {
                                    em = $({ tag: 'div', cls: 'wdk-cms-text' })[0]
                                    x.after(em);
                                }
                                x.remove();
        ...
        tool.find('a.anthor').on('mousedown', function (e) {
            ...
            Check(function (active) {
                ...
                if (range.startOffset == range.endOffset) {
                    ...
                    var href = prompt("网址", selection.toString());
                    href ? document.execCommand('createLink', false, href) : 0;
                }
                descDom.on('input');
            });
        });
        ...
        tool.find('.at_icon').on('mousedown', function (e) {
            ...
            Check(function (active) {
                if (dataKey == 'grid') {
                    ...
                    $(em).addClass('wdk-cms-grid').html('<table><tr><td></td><td></td></tr><tr><td></td><td></td></tr></table>');
                    active.after(em);
                    return;
                }
