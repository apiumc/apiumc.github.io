...
((win, $) => {
    ...
    let showDateBox = (datedom) => {
        let offset = $(inputDOM).offset()
        let thisleft = offset.left + 'px';
        let thistop = offset.top + offset.height + 5 + 'px';
        let ww = win.innerWidth;
        if (ww - offset.left < 310) {
            thisleft = (ww - 310) + 'px';
        }
        if (ww < 576) {
            thisleft = (ww / 2 - 155) + 'px'
            thistop = '25vh';
        }
        ...
        datedom.mask = $(document.createElement('div')).addClass('weui_mask')
            .click(function () {
                $(this).remove();
                close();
    ...
    let createDom = () => {
        ...
        datedom.classList.add(dateboxCls);
        datedom.tabIndex = -1;
    ...
    let bindEvent_TodayBtn = () => {
        ...
        dateboxDom.querySelector('.date-btn-today').onclick = (event) => {
            ...
            $(inputDOM).on('change', inputDOM.value);
            //
            close();
        };
    };
